...
by `flox services persist`, which warns when a service it is persisting sets
any of them. Ordering and the stop timeout have equivalents in the per-service
`systemd` passthrough — `unit.after` / `unit.requires` and
`service.timeout_stop_sec` — but there is no passthrough equivalent for
`shutdown.signal`, so a persisted unit is always stopped with systemd's
default signal.
...
service is only started after the dependencies that run alongside it, so an
edge to a service excluded from the current system by `systems` is dropped
