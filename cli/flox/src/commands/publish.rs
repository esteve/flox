...
use flox_rust_sdk::providers::catalog_lock::{self, BuildCatalogLock};
...
fn subset_for_publish(
    lock: &BuildCatalogLock,
    references: &BTreeSet<nef_lock_catalog::CatalogRef>,
) -> Result<BTreeMap<String, LockedInputEntry>> {
    lock.subset(references).map_err(|err| {
...
impl Publish {
    ...
    async fn publish(
        ...
        let references = match publish_provider.package_metadata.package.kind() {
            PackageTargetKind::ExpressionBuild(expression) => catalog_lock::package_references(
                nix_expression_dir(&path_env),
                &expression.rel_file_path,
            )?,
        ...
        // lock at all.
        let expression_rel_paths: Vec<PathBuf> =
            match publish_provider.package_metadata.package.kind() {
                ...
                PackageTargetKind::ManifestBuild { .. } => {
                    ...
                    packages_to_build(&lockfile_manifest, &expression_ref_local, &[] as &[&str])?
                        .iter()
        ...
        let catalog_lock = match expression_rel_paths.is_empty() {
            true => None,
            false => Some(
...
mod tests {
    ...
    fn stale_committed_lock_subset_names_update_catalogs() {
        ...
        .unwrap();
        let references =
            catalog_lock::package_references(expressions_dir.path(), "hello.nix").unwrap();

        let lock = catalog_lock::test_helpers::build_catalog_lock_from_parts(
