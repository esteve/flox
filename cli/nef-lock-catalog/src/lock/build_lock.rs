...
pub struct BuildLock {
    ...
    /// canonical `<catalog>/<attr-path>` form. A `BTreeMap` so the committed
    /// lock file serializes in a stable key order: refreshing a lock that
    /// resolved identically must produce an identical file.
    ///
