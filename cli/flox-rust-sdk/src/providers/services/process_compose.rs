...
use flox_manifest::parsed::v1_16_0::ServiceShutdown;
use flox_manifest::parsed::{Inner, v1_16_0};
#[cfg(test)]
...
impl From<v1_16_0::Services> for ProcessComposeConfig {
    fn from(services: v1_16_0::Services) -> Self {
...
pub fn maybe_make_service_config_file(
    ...
    let service_config_path = if !services.inner().is_empty() {
        ...
        // The lockfile carries the composed manifest, so this is the first
        // point at which a `depends-on` target contributed by an `[include]`d
        // environment resolves. A target that names no service even here is a
        // mistake no system can satisfy.
        services
            .validate_depends_on_targets()
            .map_err(|err| ServiceError::Passthrough(err.to_string()))?;
        let mut services_for_system = services.copy_for_system(&flox.system);
        // Logged rather than surfaced: a service excluded from this system by
        // `systems` is the manifest working as written, and this runs on every
        // build of the environment.
        for dropped in services_for_system.drop_depends_on_for_other_systems() {
            tracing::debug!(
                service = dropped.service,
                depends_on = dropped.target,
                system = flox.system,
                "dropped a 'depends-on' edge to a service that does not run on this system",
            );
        }
...
mod tests {
    ...
    fn service_orchestration_converts_from_manifest() {
        use flox_manifest::parsed::v1_16_0::{
            ServiceDependency,
            ServiceDescriptor,
            ServiceShutdown,
        ...
        let mut services = v1_16_0::Services::default();
        services
            .inner_mut()
        ...
        assert_eq!(config, ProcessComposeConfig {
            processes: BTreeMap::from([
                ("db".to_string(), ProcessConfig {
                    ...
                    shutdown: Some(ProcessShutdown {
                        command: Some("stop-db".to_string()),
                        timeout_seconds: NonZeroU32::new(30),
                        signal: Some(2),
                    }),
                    depends_on: None,
                }),
